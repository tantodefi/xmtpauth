...
describe("EVMAuth Contracts", function () {
  ...
  beforeEach(async function () {
    ...
    // Deploy GroupAccess
    const EVMAuthGroupAccess = await ethers.getContractFactory(
      "EVMAuthGroupAccessV2",
      {
        libraries: {},
      },
    );
  ...
  describe("GroupAccess Contract", function () {
    ...
    it("Should deploy with correct initial values", async function () {
      ...
      expect(await groupAccess.groupDescription()).to.equal(
        TEST_GROUP_DESCRIPTION,
      );
    ...
    it("Should allow users to purchase access", async function () {
      ...
      await expect(tx).to.emit(groupAccess, "UserAccessGranted");
    });
    ...
    it("Should store user inbox IDs", async function () {
      ...
      // Check mapping
      expect(await groupAccess.userInboxIds(user1.address)).to.equal(
        testInboxId,
      );
      expect(await groupAccess.inboxToAddress(testInboxId)).to.equal(
        user1.address,
      );
    });
    ...
    it("Should check access by inbox ID", async function () {
      ...
      await groupAccess
        .connect(user1)
        .purchaseAccess(tokenId, { value: price });
    ...
    it("Should calculate platform fees correctly", async function () {
      ...
      const price = ethers.parseEther("0.001");
      const expectedFee =
        (price * BigInt(INITIAL_FEE_BASIS_POINTS)) / BigInt(10000);
    ...
    it("Should not allow purchase with insufficient payment", async function () {
      ...
      await expect(
        groupAccess
          .connect(user1)
          .purchaseAccess(tokenId, { value: insufficientPrice }),
    ...
    it("Should allow admin to revoke access", async function () {
      ...
      await groupAccess
        .connect(owner)
        .revokeAccess(user1.address, tokenId, "Test revocation");
  ...
  describe("Gas Usage", function () {
    ...
    it("Should report gas usage for key operations", async function () {
      ...
      const purchaseTx = await groupAccess
        .connect(user1)
