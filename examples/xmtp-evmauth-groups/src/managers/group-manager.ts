...
export class GroupManager {
  ...
  async createPaidGroup(params: CreateGroupParams): Promise<CreateGroupResult> {
    try {
      ...
      try {
        await this.evmAuthHandler.setupTrialToken(contractAddress, params.name);
      ...
      throw new Error(
        `Failed to create paid group: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  async addUserToGroup(
    ...
    try {
      ...
      const userInboxState =
        await this.client.preferences.inboxStateFromInboxIds([userInboxId]);
      ...
      if (isAlreadyMember) {
        console.log(
          `✅ User ${userInboxId} is already a member of group ${groupId}`,
        );
        return true;
      }
  ...
  async removeUserFromGroup(
    ...
    try {
      ...
      if (group.isSuperAdmin(userInboxId) || group.isAdmin(userInboxId)) {
        console.log(
          `Cannot remove admin/super admin ${userInboxId} from group ${groupId}`,
        );
        return false;
      }
      ...
      console.log(
        `🔄 Removed user ${userInboxId} from group ${groupId}: ${reason}`,
      );
      return true;
    } catch (error) {
