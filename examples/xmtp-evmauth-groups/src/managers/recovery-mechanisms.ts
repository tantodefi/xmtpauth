...
export class RecoveryManager {
   ...
   */
  async recoverGroupConfigs(
    contractAddresses: string[],
  ): Promise<Map<string, DualGroupConfig>> {
    ...
    for (const contractAddress of contractAddresses) {
      try {
        ...
        console.error(
          `❌ Failed to recover config for ${contractAddress}:`,
          error,
        );
      }
    }
    ...
    console.log(
      `🎉 Recovery complete! Recovered ${recoveredConfigs.size} group configurations`,
    );
   ...
   */
  private async recoverSingleGroupConfig(
    contractAddress: string,
  ): Promise<DualGroupConfig | null> {
    try {
      ...
      if (
        !xmtpInfo.isActive ||
        !xmtpInfo.salesGroupId ||
        !xmtpInfo.premiumGroupId
      ) {
        console.log(
          `⚠️ Contract ${contractAddress} has no active XMTP groups linked`,
        );
        return null;
      }
   ...
   */
  async validateAndRepairMembership(
    ...
    for (const [contractAddress, config] of groupConfigs.entries()) {
      try {
        ...
        console.error(
          `Error validating membership for ${contractAddress}:`,
          error,
        );
      }
    }
  ...
  async discoverContractAddresses(): Promise<string[]> {
      ...
      for (const conversation of conversations.slice(0, 10)) {
        ...
        try {
          ...
          for (const message of messages.slice(0, 5)) {
            // Check last 5 messages per conversation
    ...
    const uniqueAddresses = [...new Set(contractAddresses)].filter(
  ...
  async performFullRecovery(): Promise<Map<string, DualGroupConfig>> {
    ...
    try {
      ...
      const recoveredConfigs =
        await this.recoverGroupConfigs(contractAddresses);
