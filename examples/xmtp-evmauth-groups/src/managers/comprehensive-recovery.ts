...
export class ComprehensiveRecovery {
  ...
  async performFullRecovery(): Promise<{
    ...
    try {
      ...
      await this.updateDatabaseWithNewContracts(
        scannedContracts,
        existingGroups,
      );

      console.log(
        `✅ Recovery complete: ${foundContracts.size} contracts found`,
      );
  ...
  private async scanConversationsForContracts(): Promise<string[]> {
    ...
    try {
      ...
      console.log(
        `🔍 Scanning ${conversations.length} conversations for contracts...`,
      );
      ...
      for (const conversation of conversations) {
        try {
          ...
          for (const message of messages) {
            if (message.senderInboxId === this.client.inboxId) {
              ...
              if (typeof content === "string") {
                ...
                if (
                  content.includes("Contract deployed at:") ||
          ...
          console.error(
            `Error scanning conversation ${conversation.id}:`,
            error,
          );
          continue;
        }
